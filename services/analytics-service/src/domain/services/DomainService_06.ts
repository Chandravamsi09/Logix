import { IDomainRepository_metrics_06 } from '../repositories/DomainRepository_06';
import { CreateDomainDTO_metrics_06, UpdateDomainDTO_metrics_06, QueryDomainFilterDTO_metrics_06 } from '../dto/DomainDTO_06';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_metrics_06 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_metrics_06) {}

  async create(dto: CreateDomainDTO_metrics_06) {
  ...
  async update(id: string, dto: UpdateDomainDTO_metrics_06) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_metrics_06) {
