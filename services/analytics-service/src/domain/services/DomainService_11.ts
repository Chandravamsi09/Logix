import { IDomainRepository_metrics_11 } from '../repositories/DomainRepository_11';
import { CreateDomainDTO_metrics_11, UpdateDomainDTO_metrics_11, QueryDomainFilterDTO_metrics_11 } from '../dto/DomainDTO_11';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_metrics_11 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_metrics_11) {}

  async create(dto: CreateDomainDTO_metrics_11) {
  ...
  async update(id: string, dto: UpdateDomainDTO_metrics_11) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_metrics_11) {
