import { IDomainRepository_metrics_14 } from '../repositories/DomainRepository_14';
import { CreateDomainDTO_metrics_14, UpdateDomainDTO_metrics_14, QueryDomainFilterDTO_metrics_14 } from '../dto/DomainDTO_14';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_metrics_14 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_metrics_14) {}

  async create(dto: CreateDomainDTO_metrics_14) {
  ...
  async update(id: string, dto: UpdateDomainDTO_metrics_14) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_metrics_14) {
