import { IDomainRepository_metrics_19 } from '../repositories/DomainRepository_19';
import { CreateDomainDTO_metrics_19, UpdateDomainDTO_metrics_19, QueryDomainFilterDTO_metrics_19 } from '../dto/DomainDTO_19';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_metrics_19 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_metrics_19) {}

  async create(dto: CreateDomainDTO_metrics_19) {
  ...
  async update(id: string, dto: UpdateDomainDTO_metrics_19) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_metrics_19) {
