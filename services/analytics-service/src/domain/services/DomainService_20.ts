import { IDomainRepository_metrics_20 } from '../repositories/DomainRepository_20';
import { CreateDomainDTO_metrics_20, UpdateDomainDTO_metrics_20, QueryDomainFilterDTO_metrics_20 } from '../dto/DomainDTO_20';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_metrics_20 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_metrics_20) {}

  async create(dto: CreateDomainDTO_metrics_20) {
  ...
  async update(id: string, dto: UpdateDomainDTO_metrics_20) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_metrics_20) {
