import { IDomainRepository_metrics_16 } from '../repositories/DomainRepository_16';
import { CreateDomainDTO_metrics_16, UpdateDomainDTO_metrics_16, QueryDomainFilterDTO_metrics_16 } from '../dto/DomainDTO_16';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_metrics_16 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_metrics_16) {}

  async create(dto: CreateDomainDTO_metrics_16) {
  ...
  async update(id: string, dto: UpdateDomainDTO_metrics_16) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_metrics_16) {
