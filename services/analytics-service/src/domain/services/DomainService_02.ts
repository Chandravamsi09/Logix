import { IDomainRepository_metrics_02 } from '../repositories/DomainRepository_02';
import { CreateDomainDTO_metrics_02, UpdateDomainDTO_metrics_02, QueryDomainFilterDTO_metrics_02 } from '../dto/DomainDTO_02';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_metrics_02 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_metrics_02) {}

  async create(dto: CreateDomainDTO_metrics_02) {
  ...
  async update(id: string, dto: UpdateDomainDTO_metrics_02) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_metrics_02) {
