import { IDomainRepository_metrics_10 } from '../repositories/DomainRepository_10';
import { CreateDomainDTO_metrics_10, UpdateDomainDTO_metrics_10, QueryDomainFilterDTO_metrics_10 } from '../dto/DomainDTO_10';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_metrics_10 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_metrics_10) {}

  async create(dto: CreateDomainDTO_metrics_10) {
  ...
  async update(id: string, dto: UpdateDomainDTO_metrics_10) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_metrics_10) {
