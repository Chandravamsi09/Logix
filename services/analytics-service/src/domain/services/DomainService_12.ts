import { IDomainRepository_metrics_12 } from '../repositories/DomainRepository_12';
import { CreateDomainDTO_metrics_12, UpdateDomainDTO_metrics_12, QueryDomainFilterDTO_metrics_12 } from '../dto/DomainDTO_12';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_metrics_12 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_metrics_12) {}

  async create(dto: CreateDomainDTO_metrics_12) {
  ...
  async update(id: string, dto: UpdateDomainDTO_metrics_12) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_metrics_12) {
