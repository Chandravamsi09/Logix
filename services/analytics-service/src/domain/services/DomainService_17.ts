import { IDomainRepository_metrics_17 } from '../repositories/DomainRepository_17';
import { CreateDomainDTO_metrics_17, UpdateDomainDTO_metrics_17, QueryDomainFilterDTO_metrics_17 } from '../dto/DomainDTO_17';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_metrics_17 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_metrics_17) {}

  async create(dto: CreateDomainDTO_metrics_17) {
  ...
  async update(id: string, dto: UpdateDomainDTO_metrics_17) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_metrics_17) {
