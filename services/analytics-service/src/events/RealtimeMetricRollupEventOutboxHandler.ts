/**
 * Event-Driven Architecture Outbox & Domain Event Handler: analytics-service
 * Stream metrics, latency percentiles, and SLA risk alerts
 */

export interface IRealtimeMetricRollupEvent {
  eventId: string;
  eventType: string;
  aggregateId: string;
  tenantId: string;
...
export interface IOutboxMessage {
  ...
  partitionKey: string;
  eventPayload: IRealtimeMetricRollupEvent;
...
export class RealtimeMetricRollupEventOutboxHandler {
  private readonly outboxQueue: IOutboxMessage[] = [];
  private readonly subscribers = new Map<string, Array<(event: IRealtimeMetricRollupEvent) => Promise<void>>>();

  public queueEvent(topic: string, event: IRealtimeMetricRollupEvent): IOutboxMessage {
  ...
  public subscribe(eventType: string, handler: (event: IRealtimeMetricRollupEvent) => Promise<void>): void {
