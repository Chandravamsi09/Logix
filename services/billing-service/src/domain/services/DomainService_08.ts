import { IDomainRepository_ledger_08 } from '../repositories/DomainRepository_08';
import { CreateDomainDTO_ledger_08, UpdateDomainDTO_ledger_08, QueryDomainFilterDTO_ledger_08 } from '../dto/DomainDTO_08';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_ledger_08 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_ledger_08) {}

  async create(dto: CreateDomainDTO_ledger_08) {
  ...
  async update(id: string, dto: UpdateDomainDTO_ledger_08) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_ledger_08) {
