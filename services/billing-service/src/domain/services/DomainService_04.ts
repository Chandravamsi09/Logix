import { IDomainRepository_ledger_04 } from '../repositories/DomainRepository_04';
import { CreateDomainDTO_ledger_04, UpdateDomainDTO_ledger_04, QueryDomainFilterDTO_ledger_04 } from '../dto/DomainDTO_04';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_ledger_04 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_ledger_04) {}

  async create(dto: CreateDomainDTO_ledger_04) {
  ...
  async update(id: string, dto: UpdateDomainDTO_ledger_04) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_ledger_04) {
