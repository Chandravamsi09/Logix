import { IDomainRepository_ledger_09 } from '../repositories/DomainRepository_09';
import { CreateDomainDTO_ledger_09, UpdateDomainDTO_ledger_09, QueryDomainFilterDTO_ledger_09 } from '../dto/DomainDTO_09';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_ledger_09 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_ledger_09) {}

  async create(dto: CreateDomainDTO_ledger_09) {
  ...
  async update(id: string, dto: UpdateDomainDTO_ledger_09) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_ledger_09) {
