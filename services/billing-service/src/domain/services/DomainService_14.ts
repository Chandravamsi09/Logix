import { IDomainRepository_ledger_14 } from '../repositories/DomainRepository_14';
import { CreateDomainDTO_ledger_14, UpdateDomainDTO_ledger_14, QueryDomainFilterDTO_ledger_14 } from '../dto/DomainDTO_14';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_ledger_14 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_ledger_14) {}

  async create(dto: CreateDomainDTO_ledger_14) {
  ...
  async update(id: string, dto: UpdateDomainDTO_ledger_14) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_ledger_14) {
