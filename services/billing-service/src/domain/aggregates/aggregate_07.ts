...
export class AggregateRoot_billing_07 {
  private state: AggregateState_billing_07;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.07.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_billing_07['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.07.status_transitioned',
