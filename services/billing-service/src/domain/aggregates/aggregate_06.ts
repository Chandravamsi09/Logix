...
export class AggregateRoot_billing_06 {
  private state: AggregateState_billing_06;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.06.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_billing_06['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.06.status_transitioned',
