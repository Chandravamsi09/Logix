...
export class AggregateRoot_billing_10 {
  private state: AggregateState_billing_10;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.10.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_billing_10['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.10.status_transitioned',
