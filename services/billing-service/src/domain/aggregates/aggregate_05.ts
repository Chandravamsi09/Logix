...
export class AggregateRoot_billing_05 {
  private state: AggregateState_billing_05;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.05.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_billing_05['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.05.status_transitioned',
