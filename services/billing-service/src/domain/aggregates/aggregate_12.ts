...
export class AggregateRoot_billing_12 {
  private state: AggregateState_billing_12;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.12.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_billing_12['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.12.status_transitioned',
