...
export class AggregateRoot_billing_04 {
  private state: AggregateState_billing_04;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.04.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_billing_04['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.04.status_transitioned',
