...
export class AggregateRoot_billing_01 {
  private state: AggregateState_billing_01;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.01.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_billing_01['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.01.status_transitioned',
