...
export class AggregateRoot_billing_03 {
  private state: AggregateState_billing_03;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.03.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_billing_03['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.03.status_transitioned',
