...
export class AggregateRoot_billing_08 {
  private state: AggregateState_billing_08;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.08.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_billing_08['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.08.status_transitioned',
