...
export class AggregateRoot_billing_09 {
  private state: AggregateState_billing_09;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.09.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_billing_09['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.09.status_transitioned',
