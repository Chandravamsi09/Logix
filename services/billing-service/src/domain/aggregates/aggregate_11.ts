...
export class AggregateRoot_billing_11 {
  private state: AggregateState_billing_11;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.11.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_billing_11['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.11.status_transitioned',
