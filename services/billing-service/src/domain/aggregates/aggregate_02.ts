...
export class AggregateRoot_billing_02 {
  private state: AggregateState_billing_02;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.02.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_billing_02['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'billing.02.status_transitioned',
