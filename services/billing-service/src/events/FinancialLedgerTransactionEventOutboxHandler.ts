/**
 * Event-Driven Architecture Outbox & Domain Event Handler: billing-service
 * Invoices created, payments settled, and general ledger journal postings
 */

export interface IFinancialLedgerTransactionEvent {
  eventId: string;
  eventType: string;
  aggregateId: string;
  tenantId: string;
...
export interface IOutboxMessage {
  ...
  partitionKey: string;
  eventPayload: IFinancialLedgerTransactionEvent;
...
export class FinancialLedgerTransactionEventOutboxHandler {
  private readonly outboxQueue: IOutboxMessage[] = [];
  private readonly subscribers = new Map<string, Array<(event: IFinancialLedgerTransactionEvent) => Promise<void>>>();

  public queueEvent(topic: string, event: IFinancialLedgerTransactionEvent): IOutboxMessage {
  ...
  public subscribe(eventType: string, handler: (event: IFinancialLedgerTransactionEvent) => Promise<void>): void {
