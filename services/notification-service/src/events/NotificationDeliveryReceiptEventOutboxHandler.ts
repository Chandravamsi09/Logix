/**
 * Event-Driven Architecture Outbox & Domain Event Handler: notification-service
 * SMS, Push, and Email gateway webhook delivery confirmations
 */

export interface INotificationDeliveryReceiptEvent {
  eventId: string;
  eventType: string;
  aggregateId: string;
  tenantId: string;
...
export interface IOutboxMessage {
  ...
  partitionKey: string;
  eventPayload: INotificationDeliveryReceiptEvent;
...
export class NotificationDeliveryReceiptEventOutboxHandler {
  private readonly outboxQueue: IOutboxMessage[] = [];
  private readonly subscribers = new Map<string, Array<(event: INotificationDeliveryReceiptEvent) => Promise<void>>>();

  public queueEvent(topic: string, event: INotificationDeliveryReceiptEvent): IOutboxMessage {
  ...
  public subscribe(eventType: string, handler: (event: INotificationDeliveryReceiptEvent) => Promise<void>): void {
