import { IDomainRepository_alerting_02 } from '../repositories/DomainRepository_02';
import { CreateDomainDTO_alerting_02, UpdateDomainDTO_alerting_02, QueryDomainFilterDTO_alerting_02 } from '../dto/DomainDTO_02';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_alerting_02 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_alerting_02) {}

  async create(dto: CreateDomainDTO_alerting_02) {
  ...
  async update(id: string, dto: UpdateDomainDTO_alerting_02) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_alerting_02) {
