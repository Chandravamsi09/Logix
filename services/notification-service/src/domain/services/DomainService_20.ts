import { IDomainRepository_alerting_20 } from '../repositories/DomainRepository_20';
import { CreateDomainDTO_alerting_20, UpdateDomainDTO_alerting_20, QueryDomainFilterDTO_alerting_20 } from '../dto/DomainDTO_20';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_alerting_20 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_alerting_20) {}

  async create(dto: CreateDomainDTO_alerting_20) {
  ...
  async update(id: string, dto: UpdateDomainDTO_alerting_20) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_alerting_20) {
