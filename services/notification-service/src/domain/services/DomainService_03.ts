import { IDomainRepository_alerting_03 } from '../repositories/DomainRepository_03';
import { CreateDomainDTO_alerting_03, UpdateDomainDTO_alerting_03, QueryDomainFilterDTO_alerting_03 } from '../dto/DomainDTO_03';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_alerting_03 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_alerting_03) {}

  async create(dto: CreateDomainDTO_alerting_03) {
  ...
  async update(id: string, dto: UpdateDomainDTO_alerting_03) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_alerting_03) {
