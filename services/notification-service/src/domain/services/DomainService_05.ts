import { IDomainRepository_alerting_05 } from '../repositories/DomainRepository_05';
import { CreateDomainDTO_alerting_05, UpdateDomainDTO_alerting_05, QueryDomainFilterDTO_alerting_05 } from '../dto/DomainDTO_05';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_alerting_05 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_alerting_05) {}

  async create(dto: CreateDomainDTO_alerting_05) {
  ...
  async update(id: string, dto: UpdateDomainDTO_alerting_05) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_alerting_05) {
