import { IDomainRepository_alerting_04 } from '../repositories/DomainRepository_04';
import { CreateDomainDTO_alerting_04, UpdateDomainDTO_alerting_04, QueryDomainFilterDTO_alerting_04 } from '../dto/DomainDTO_04';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_alerting_04 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_alerting_04) {}

  async create(dto: CreateDomainDTO_alerting_04) {
  ...
  async update(id: string, dto: UpdateDomainDTO_alerting_04) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_alerting_04) {
