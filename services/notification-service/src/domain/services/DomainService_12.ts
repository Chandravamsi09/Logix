import { IDomainRepository_alerting_12 } from '../repositories/DomainRepository_12';
import { CreateDomainDTO_alerting_12, UpdateDomainDTO_alerting_12, QueryDomainFilterDTO_alerting_12 } from '../dto/DomainDTO_12';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_alerting_12 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_alerting_12) {}

  async create(dto: CreateDomainDTO_alerting_12) {
  ...
  async update(id: string, dto: UpdateDomainDTO_alerting_12) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_alerting_12) {
