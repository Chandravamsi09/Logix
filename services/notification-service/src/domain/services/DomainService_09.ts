import { IDomainRepository_alerting_09 } from '../repositories/DomainRepository_09';
import { CreateDomainDTO_alerting_09, UpdateDomainDTO_alerting_09, QueryDomainFilterDTO_alerting_09 } from '../dto/DomainDTO_09';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_alerting_09 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_alerting_09) {}

  async create(dto: CreateDomainDTO_alerting_09) {
  ...
  async update(id: string, dto: UpdateDomainDTO_alerting_09) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_alerting_09) {
