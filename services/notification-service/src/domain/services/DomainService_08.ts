import { IDomainRepository_alerting_08 } from '../repositories/DomainRepository_08';
import { CreateDomainDTO_alerting_08, UpdateDomainDTO_alerting_08, QueryDomainFilterDTO_alerting_08 } from '../dto/DomainDTO_08';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_alerting_08 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_alerting_08) {}

  async create(dto: CreateDomainDTO_alerting_08) {
  ...
  async update(id: string, dto: UpdateDomainDTO_alerting_08) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_alerting_08) {
