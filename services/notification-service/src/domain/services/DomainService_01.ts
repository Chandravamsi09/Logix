import { IDomainRepository_alerting_01 } from '../repositories/DomainRepository_01';
import { CreateDomainDTO_alerting_01, UpdateDomainDTO_alerting_01, QueryDomainFilterDTO_alerting_01 } from '../dto/DomainDTO_01';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_alerting_01 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_alerting_01) {}

  async create(dto: CreateDomainDTO_alerting_01) {
  ...
  async update(id: string, dto: UpdateDomainDTO_alerting_01) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_alerting_01) {
