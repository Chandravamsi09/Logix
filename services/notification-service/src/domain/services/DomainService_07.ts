import { IDomainRepository_alerting_07 } from '../repositories/DomainRepository_07';
import { CreateDomainDTO_alerting_07, UpdateDomainDTO_alerting_07, QueryDomainFilterDTO_alerting_07 } from '../dto/DomainDTO_07';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_alerting_07 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_alerting_07) {}

  async create(dto: CreateDomainDTO_alerting_07) {
  ...
  async update(id: string, dto: UpdateDomainDTO_alerting_07) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_alerting_07) {
