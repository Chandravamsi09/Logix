...
export class AggregateRoot_order_01 {
  private state: AggregateState_order_01;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.01.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_order_01['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.01.status_transitioned',
