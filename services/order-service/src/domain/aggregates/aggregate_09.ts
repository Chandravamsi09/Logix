...
export class AggregateRoot_order_09 {
  private state: AggregateState_order_09;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.09.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_order_09['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.09.status_transitioned',
