...
export class AggregateRoot_order_02 {
  private state: AggregateState_order_02;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.02.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_order_02['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.02.status_transitioned',
