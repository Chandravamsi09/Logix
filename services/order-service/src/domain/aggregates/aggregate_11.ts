...
export class AggregateRoot_order_11 {
  private state: AggregateState_order_11;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.11.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_order_11['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.11.status_transitioned',
