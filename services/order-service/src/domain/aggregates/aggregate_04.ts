...
export class AggregateRoot_order_04 {
  private state: AggregateState_order_04;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.04.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_order_04['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.04.status_transitioned',
