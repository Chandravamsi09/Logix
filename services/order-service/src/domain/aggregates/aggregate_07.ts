...
export class AggregateRoot_order_07 {
  private state: AggregateState_order_07;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.07.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_order_07['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.07.status_transitioned',
