...
export class AggregateRoot_order_06 {
  private state: AggregateState_order_06;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.06.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_order_06['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.06.status_transitioned',
