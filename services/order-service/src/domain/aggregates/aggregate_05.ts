...
export class AggregateRoot_order_05 {
  private state: AggregateState_order_05;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.05.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_order_05['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.05.status_transitioned',
