...
export class AggregateRoot_order_03 {
  private state: AggregateState_order_03;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.03.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_order_03['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.03.status_transitioned',
