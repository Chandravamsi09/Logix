...
export class AggregateRoot_order_10 {
  private state: AggregateState_order_10;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.10.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_order_10['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.10.status_transitioned',
