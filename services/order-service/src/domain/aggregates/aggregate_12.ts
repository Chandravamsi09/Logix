...
export class AggregateRoot_order_12 {
  private state: AggregateState_order_12;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.12.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_order_12['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.12.status_transitioned',
