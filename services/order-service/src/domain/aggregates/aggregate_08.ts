...
export class AggregateRoot_order_08 {
  private state: AggregateState_order_08;
  ...
  public setAttribute(key: string, value: string | number | boolean): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.08.attribute_updated',
      payload: { key, value },
      timestamp: new Date()
    });
  }
  ...
  public transitionStatus(newStatus: AggregateState_order_08['operationalStatus']): void {
    ...
    this.uncommittedEvents.push({
      eventType: 'order.08.status_transitioned',
