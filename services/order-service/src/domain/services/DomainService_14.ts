import { IDomainRepository_orders_14 } from '../repositories/DomainRepository_14';
import { CreateDomainDTO_orders_14, UpdateDomainDTO_orders_14, QueryDomainFilterDTO_orders_14 } from '../dto/DomainDTO_14';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_orders_14 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_orders_14) {}

  async create(dto: CreateDomainDTO_orders_14) {
  ...
  async update(id: string, dto: UpdateDomainDTO_orders_14) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_orders_14) {
