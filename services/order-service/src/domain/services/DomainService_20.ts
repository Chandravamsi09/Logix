import { IDomainRepository_orders_20 } from '../repositories/DomainRepository_20';
import { CreateDomainDTO_orders_20, UpdateDomainDTO_orders_20, QueryDomainFilterDTO_orders_20 } from '../dto/DomainDTO_20';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_orders_20 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_orders_20) {}

  async create(dto: CreateDomainDTO_orders_20) {
  ...
  async update(id: string, dto: UpdateDomainDTO_orders_20) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_orders_20) {
