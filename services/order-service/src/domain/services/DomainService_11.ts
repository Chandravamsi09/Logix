import { IDomainRepository_orders_11 } from '../repositories/DomainRepository_11';
import { CreateDomainDTO_orders_11, UpdateDomainDTO_orders_11, QueryDomainFilterDTO_orders_11 } from '../dto/DomainDTO_11';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_orders_11 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_orders_11) {}

  async create(dto: CreateDomainDTO_orders_11) {
  ...
  async update(id: string, dto: UpdateDomainDTO_orders_11) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_orders_11) {
