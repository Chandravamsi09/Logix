import { IDomainRepository_orders_07 } from '../repositories/DomainRepository_07';
import { CreateDomainDTO_orders_07, UpdateDomainDTO_orders_07, QueryDomainFilterDTO_orders_07 } from '../dto/DomainDTO_07';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_orders_07 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_orders_07) {}

  async create(dto: CreateDomainDTO_orders_07) {
  ...
  async update(id: string, dto: UpdateDomainDTO_orders_07) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_orders_07) {
