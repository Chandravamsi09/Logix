import { IDomainRepository_orders_16 } from '../repositories/DomainRepository_16';
import { CreateDomainDTO_orders_16, UpdateDomainDTO_orders_16, QueryDomainFilterDTO_orders_16 } from '../dto/DomainDTO_16';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_orders_16 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_orders_16) {}

  async create(dto: CreateDomainDTO_orders_16) {
  ...
  async update(id: string, dto: UpdateDomainDTO_orders_16) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_orders_16) {
