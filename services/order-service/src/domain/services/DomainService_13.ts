import { IDomainRepository_orders_13 } from '../repositories/DomainRepository_13';
import { CreateDomainDTO_orders_13, UpdateDomainDTO_orders_13, QueryDomainFilterDTO_orders_13 } from '../dto/DomainDTO_13';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_orders_13 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_orders_13) {}

  async create(dto: CreateDomainDTO_orders_13) {
  ...
  async update(id: string, dto: UpdateDomainDTO_orders_13) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_orders_13) {
