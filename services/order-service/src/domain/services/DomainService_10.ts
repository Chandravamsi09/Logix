import { IDomainRepository_orders_10 } from '../repositories/DomainRepository_10';
import { CreateDomainDTO_orders_10, UpdateDomainDTO_orders_10, QueryDomainFilterDTO_orders_10 } from '../dto/DomainDTO_10';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_orders_10 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_orders_10) {}

  async create(dto: CreateDomainDTO_orders_10) {
  ...
  async update(id: string, dto: UpdateDomainDTO_orders_10) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_orders_10) {
