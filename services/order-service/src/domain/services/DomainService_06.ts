import { IDomainRepository_orders_06 } from '../repositories/DomainRepository_06';
import { CreateDomainDTO_orders_06, UpdateDomainDTO_orders_06, QueryDomainFilterDTO_orders_06 } from '../dto/DomainDTO_06';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_orders_06 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_orders_06) {}

  async create(dto: CreateDomainDTO_orders_06) {
  ...
  async update(id: string, dto: UpdateDomainDTO_orders_06) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_orders_06) {
