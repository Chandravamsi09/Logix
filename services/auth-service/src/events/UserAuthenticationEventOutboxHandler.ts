/**
 * Event-Driven Architecture Outbox & Domain Event Handler: auth-service
 * Auth state audit telemetry
 */

export interface IUserAuthenticationEvent {
  eventId: string;
  eventType: string;
  aggregateId: string;
  tenantId: string;
...
export interface IOutboxMessage {
  ...
  partitionKey: string;
  eventPayload: IUserAuthenticationEvent;
...
export class UserAuthenticationEventOutboxHandler {
  private readonly outboxQueue: IOutboxMessage[] = [];
  private readonly subscribers = new Map<string, Array<(event: IUserAuthenticationEvent) => Promise<void>>>();

  public queueEvent(topic: string, event: IUserAuthenticationEvent): IOutboxMessage {
  ...
  public subscribe(eventType: string, handler: (event: IUserAuthenticationEvent) => Promise<void>): void {
