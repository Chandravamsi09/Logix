...
export class EnterpriseModule_auth_017 {
  private readonly units = new Map<string, IDomainUnit_auth_017>();

  constructor(public readonly moduleNamespace: string = 'AUTH_NS_017') {}
  ...
  ): IDomainUnit_auth_017 {
    const unit: IDomainUnit_auth_017 = {
      unitId,
      tenantId,
      operationalCode,
      executionPriority,
  ...
  public transitionState(
    unitId: string,
    newState: IDomainUnit_auth_017['state'],
    actorUserId: string
  ): boolean {
    const unit = this.units.get(unitId);
    if (!unit) return false;
  ...
  public getUnit(unitId: string): IDomainUnit_auth_017 | null {
    return this.units.get(unitId) || null;
  }

  public listUnitsByTenant(tenantId: string): IDomainUnit_auth_017[] {
