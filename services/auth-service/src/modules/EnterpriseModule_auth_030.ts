...
export class EnterpriseModule_auth_030 {
  private readonly units = new Map<string, IDomainUnit_auth_030>();

  constructor(public readonly moduleNamespace: string = 'AUTH_NS_030') {}
  ...
  ): IDomainUnit_auth_030 {
    const unit: IDomainUnit_auth_030 = {
      unitId,
      tenantId,
      operationalCode,
      executionPriority,
  ...
  public transitionState(
    unitId: string,
    newState: IDomainUnit_auth_030['state'],
    actorUserId: string
  ): boolean {
    const unit = this.units.get(unitId);
    if (!unit) return false;
  ...
  public getUnit(unitId: string): IDomainUnit_auth_030 | null {
    return this.units.get(unitId) || null;
  }

  public listUnitsByTenant(tenantId: string): IDomainUnit_auth_030[] {
