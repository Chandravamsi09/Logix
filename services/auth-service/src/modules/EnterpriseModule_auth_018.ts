...
export class EnterpriseModule_auth_018 {
  private readonly units = new Map<string, IDomainUnit_auth_018>();

  constructor(public readonly moduleNamespace: string = 'AUTH_NS_018') {}
  ...
  ): IDomainUnit_auth_018 {
    const unit: IDomainUnit_auth_018 = {
      unitId,
      tenantId,
      operationalCode,
      executionPriority,
  ...
  public transitionState(
    unitId: string,
    newState: IDomainUnit_auth_018['state'],
    actorUserId: string
  ): boolean {
    const unit = this.units.get(unitId);
    if (!unit) return false;
  ...
  public getUnit(unitId: string): IDomainUnit_auth_018 | null {
    return this.units.get(unitId) || null;
  }

  public listUnitsByTenant(tenantId: string): IDomainUnit_auth_018[] {
