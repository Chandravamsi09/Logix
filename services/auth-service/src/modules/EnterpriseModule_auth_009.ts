...
export class EnterpriseModule_auth_009 {
  private readonly units = new Map<string, IDomainUnit_auth_009>();

  constructor(public readonly moduleNamespace: string = 'AUTH_NS_009') {}
  ...
  ): IDomainUnit_auth_009 {
    const unit: IDomainUnit_auth_009 = {
      unitId,
      tenantId,
      operationalCode,
      executionPriority,
  ...
  public transitionState(
    unitId: string,
    newState: IDomainUnit_auth_009['state'],
    actorUserId: string
  ): boolean {
    const unit = this.units.get(unitId);
    if (!unit) return false;
  ...
  public getUnit(unitId: string): IDomainUnit_auth_009 | null {
    return this.units.get(unitId) || null;
  }

  public listUnitsByTenant(tenantId: string): IDomainUnit_auth_009[] {
