/**
 * Logix Enterprise Domain Entity: IAM - Section 20
 * Strictly typed domain model with invariant guardrails and immutability assertions.
 */

export interface IDomainEntity_iam_20 {
  id: string;
  tenantId: string;
  code: string;
  displayName: string;
...
export class DomainEntity_iam_20 implements IDomainEntity_iam_20 {
  public id: string;
  ...
  constructor(data: Partial<IDomainEntity_iam_20>) {
  ...
  public toDTO(): IDomainEntity_iam_20 {
    return {
      id: this.id,
