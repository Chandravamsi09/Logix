import { IDomainRepository_iam_09 } from '../repositories/DomainRepository_09';
import { CreateDomainDTO_iam_09, UpdateDomainDTO_iam_09, QueryDomainFilterDTO_iam_09 } from '../dto/DomainDTO_09';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_iam_09 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_iam_09) {}

  async create(dto: CreateDomainDTO_iam_09) {
  ...
  async update(id: string, dto: UpdateDomainDTO_iam_09) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_iam_09) {
