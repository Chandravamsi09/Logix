import { IDomainRepository_iam_02 } from '../repositories/DomainRepository_02';
import { CreateDomainDTO_iam_02, UpdateDomainDTO_iam_02, QueryDomainFilterDTO_iam_02 } from '../dto/DomainDTO_02';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_iam_02 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_iam_02) {}

  async create(dto: CreateDomainDTO_iam_02) {
  ...
  async update(id: string, dto: UpdateDomainDTO_iam_02) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_iam_02) {
