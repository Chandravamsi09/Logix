import { IDomainRepository_iam_03 } from '../repositories/DomainRepository_03';
import { CreateDomainDTO_iam_03, UpdateDomainDTO_iam_03, QueryDomainFilterDTO_iam_03 } from '../dto/DomainDTO_03';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_iam_03 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_iam_03) {}

  async create(dto: CreateDomainDTO_iam_03) {
  ...
  async update(id: string, dto: UpdateDomainDTO_iam_03) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_iam_03) {
