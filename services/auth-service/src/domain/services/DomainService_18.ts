import { IDomainRepository_iam_18 } from '../repositories/DomainRepository_18';
import { CreateDomainDTO_iam_18, UpdateDomainDTO_iam_18, QueryDomainFilterDTO_iam_18 } from '../dto/DomainDTO_18';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_iam_18 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_iam_18) {}

  async create(dto: CreateDomainDTO_iam_18) {
  ...
  async update(id: string, dto: UpdateDomainDTO_iam_18) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_iam_18) {
