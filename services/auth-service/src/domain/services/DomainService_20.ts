import { IDomainRepository_iam_20 } from '../repositories/DomainRepository_20';
import { CreateDomainDTO_iam_20, UpdateDomainDTO_iam_20, QueryDomainFilterDTO_iam_20 } from '../dto/DomainDTO_20';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_iam_20 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_iam_20) {}

  async create(dto: CreateDomainDTO_iam_20) {
  ...
  async update(id: string, dto: UpdateDomainDTO_iam_20) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_iam_20) {
