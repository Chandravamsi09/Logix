import { IDomainRepository_iam_16 } from '../repositories/DomainRepository_16';
import { CreateDomainDTO_iam_16, UpdateDomainDTO_iam_16, QueryDomainFilterDTO_iam_16 } from '../dto/DomainDTO_16';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_iam_16 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_iam_16) {}

  async create(dto: CreateDomainDTO_iam_16) {
  ...
  async update(id: string, dto: UpdateDomainDTO_iam_16) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_iam_16) {
