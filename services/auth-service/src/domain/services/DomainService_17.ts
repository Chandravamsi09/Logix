import { IDomainRepository_iam_17 } from '../repositories/DomainRepository_17';
import { CreateDomainDTO_iam_17, UpdateDomainDTO_iam_17, QueryDomainFilterDTO_iam_17 } from '../dto/DomainDTO_17';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_iam_17 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_iam_17) {}

  async create(dto: CreateDomainDTO_iam_17) {
  ...
  async update(id: string, dto: UpdateDomainDTO_iam_17) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_iam_17) {
