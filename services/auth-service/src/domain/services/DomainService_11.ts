import { IDomainRepository_iam_11 } from '../repositories/DomainRepository_11';
import { CreateDomainDTO_iam_11, UpdateDomainDTO_iam_11, QueryDomainFilterDTO_iam_11 } from '../dto/DomainDTO_11';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_iam_11 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_iam_11) {}

  async create(dto: CreateDomainDTO_iam_11) {
  ...
  async update(id: string, dto: UpdateDomainDTO_iam_11) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_iam_11) {
