import { IDomainRepository_iam_06 } from '../repositories/DomainRepository_06';
import { CreateDomainDTO_iam_06, UpdateDomainDTO_iam_06, QueryDomainFilterDTO_iam_06 } from '../dto/DomainDTO_06';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_iam_06 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_iam_06) {}

  async create(dto: CreateDomainDTO_iam_06) {
  ...
  async update(id: string, dto: UpdateDomainDTO_iam_06) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_iam_06) {
