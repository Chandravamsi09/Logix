import { IDomainRepository_iam_14 } from '../repositories/DomainRepository_14';
import { CreateDomainDTO_iam_14, UpdateDomainDTO_iam_14, QueryDomainFilterDTO_iam_14 } from '../dto/DomainDTO_14';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_iam_14 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_iam_14) {}

  async create(dto: CreateDomainDTO_iam_14) {
  ...
  async update(id: string, dto: UpdateDomainDTO_iam_14) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_iam_14) {
