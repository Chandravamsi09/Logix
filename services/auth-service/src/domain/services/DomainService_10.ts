import { IDomainRepository_iam_10 } from '../repositories/DomainRepository_10';
import { CreateDomainDTO_iam_10, UpdateDomainDTO_iam_10, QueryDomainFilterDTO_iam_10 } from '../dto/DomainDTO_10';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_iam_10 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_iam_10) {}

  async create(dto: CreateDomainDTO_iam_10) {
  ...
  async update(id: string, dto: UpdateDomainDTO_iam_10) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_iam_10) {
