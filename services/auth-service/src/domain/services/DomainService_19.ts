import { IDomainRepository_iam_19 } from '../repositories/DomainRepository_19';
import { CreateDomainDTO_iam_19, UpdateDomainDTO_iam_19, QueryDomainFilterDTO_iam_19 } from '../dto/DomainDTO_19';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_iam_19 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_iam_19) {}

  async create(dto: CreateDomainDTO_iam_19) {
  ...
  async update(id: string, dto: UpdateDomainDTO_iam_19) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_iam_19) {
