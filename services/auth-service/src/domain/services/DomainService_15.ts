import { IDomainRepository_iam_15 } from '../repositories/DomainRepository_15';
import { CreateDomainDTO_iam_15, UpdateDomainDTO_iam_15, QueryDomainFilterDTO_iam_15 } from '../dto/DomainDTO_15';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_iam_15 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_iam_15) {}

  async create(dto: CreateDomainDTO_iam_15) {
  ...
  async update(id: string, dto: UpdateDomainDTO_iam_15) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_iam_15) {
