/**
 * Logix Enterprise Domain Entity: WAREHOUSE - Section 20
 * Strictly typed domain model with invariant guardrails and immutability assertions.
 */

export interface IDomainEntity_warehouse_20 {
  id: string;
  tenantId: string;
  code: string;
  displayName: string;
...
export class DomainEntity_warehouse_20 implements IDomainEntity_warehouse_20 {
  public id: string;
  ...
  constructor(data: Partial<IDomainEntity_warehouse_20>) {
  ...
  public toDTO(): IDomainEntity_warehouse_20 {
    return {
      id: this.id,
