import { IDomainRepository_warehouse_11 } from '../repositories/DomainRepository_11';
import { CreateDomainDTO_warehouse_11, UpdateDomainDTO_warehouse_11, QueryDomainFilterDTO_warehouse_11 } from '../dto/DomainDTO_11';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_warehouse_11 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_warehouse_11) {}

  async create(dto: CreateDomainDTO_warehouse_11) {
  ...
  async update(id: string, dto: UpdateDomainDTO_warehouse_11) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_warehouse_11) {
