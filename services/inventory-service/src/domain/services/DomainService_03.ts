import { IDomainRepository_warehouse_03 } from '../repositories/DomainRepository_03';
import { CreateDomainDTO_warehouse_03, UpdateDomainDTO_warehouse_03, QueryDomainFilterDTO_warehouse_03 } from '../dto/DomainDTO_03';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_warehouse_03 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_warehouse_03) {}

  async create(dto: CreateDomainDTO_warehouse_03) {
  ...
  async update(id: string, dto: UpdateDomainDTO_warehouse_03) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_warehouse_03) {
