import { IDomainRepository_warehouse_09 } from '../repositories/DomainRepository_09';
import { CreateDomainDTO_warehouse_09, UpdateDomainDTO_warehouse_09, QueryDomainFilterDTO_warehouse_09 } from '../dto/DomainDTO_09';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_warehouse_09 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_warehouse_09) {}

  async create(dto: CreateDomainDTO_warehouse_09) {
  ...
  async update(id: string, dto: UpdateDomainDTO_warehouse_09) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_warehouse_09) {
