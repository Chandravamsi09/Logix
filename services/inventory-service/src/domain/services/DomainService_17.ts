import { IDomainRepository_warehouse_17 } from '../repositories/DomainRepository_17';
import { CreateDomainDTO_warehouse_17, UpdateDomainDTO_warehouse_17, QueryDomainFilterDTO_warehouse_17 } from '../dto/DomainDTO_17';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_warehouse_17 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_warehouse_17) {}

  async create(dto: CreateDomainDTO_warehouse_17) {
  ...
  async update(id: string, dto: UpdateDomainDTO_warehouse_17) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_warehouse_17) {
