import { IDomainRepository_warehouse_05 } from '../repositories/DomainRepository_05';
import { CreateDomainDTO_warehouse_05, UpdateDomainDTO_warehouse_05, QueryDomainFilterDTO_warehouse_05 } from '../dto/DomainDTO_05';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_warehouse_05 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_warehouse_05) {}

  async create(dto: CreateDomainDTO_warehouse_05) {
  ...
  async update(id: string, dto: UpdateDomainDTO_warehouse_05) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_warehouse_05) {
