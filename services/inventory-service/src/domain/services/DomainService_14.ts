import { IDomainRepository_warehouse_14 } from '../repositories/DomainRepository_14';
import { CreateDomainDTO_warehouse_14, UpdateDomainDTO_warehouse_14, QueryDomainFilterDTO_warehouse_14 } from '../dto/DomainDTO_14';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_warehouse_14 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_warehouse_14) {}

  async create(dto: CreateDomainDTO_warehouse_14) {
  ...
  async update(id: string, dto: UpdateDomainDTO_warehouse_14) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_warehouse_14) {
