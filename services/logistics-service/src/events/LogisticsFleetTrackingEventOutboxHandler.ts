/**
 * Event-Driven Architecture Outbox & Domain Event Handler: logistics-service
 * Vehicle GPS pings, waypoint check-ins, and ePOD sign-offs
 */

export interface ILogisticsFleetTrackingEvent {
  eventId: string;
  eventType: string;
  aggregateId: string;
  tenantId: string;
...
export interface IOutboxMessage {
  ...
  partitionKey: string;
  eventPayload: ILogisticsFleetTrackingEvent;
...
export class LogisticsFleetTrackingEventOutboxHandler {
  private readonly outboxQueue: IOutboxMessage[] = [];
  private readonly subscribers = new Map<string, Array<(event: ILogisticsFleetTrackingEvent) => Promise<void>>>();

  public queueEvent(topic: string, event: ILogisticsFleetTrackingEvent): IOutboxMessage {
  ...
  public subscribe(eventType: string, handler: (event: ILogisticsFleetTrackingEvent) => Promise<void>): void {
