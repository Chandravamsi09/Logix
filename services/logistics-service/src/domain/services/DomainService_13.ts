import { IDomainRepository_fleet_13 } from '../repositories/DomainRepository_13';
import { CreateDomainDTO_fleet_13, UpdateDomainDTO_fleet_13, QueryDomainFilterDTO_fleet_13 } from '../dto/DomainDTO_13';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_fleet_13 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_fleet_13) {}

  async create(dto: CreateDomainDTO_fleet_13) {
  ...
  async update(id: string, dto: UpdateDomainDTO_fleet_13) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_fleet_13) {
