import { IDomainRepository_fleet_16 } from '../repositories/DomainRepository_16';
import { CreateDomainDTO_fleet_16, UpdateDomainDTO_fleet_16, QueryDomainFilterDTO_fleet_16 } from '../dto/DomainDTO_16';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_fleet_16 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_fleet_16) {}

  async create(dto: CreateDomainDTO_fleet_16) {
  ...
  async update(id: string, dto: UpdateDomainDTO_fleet_16) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_fleet_16) {
