import { IDomainRepository_fleet_10 } from '../repositories/DomainRepository_10';
import { CreateDomainDTO_fleet_10, UpdateDomainDTO_fleet_10, QueryDomainFilterDTO_fleet_10 } from '../dto/DomainDTO_10';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_fleet_10 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_fleet_10) {}

  async create(dto: CreateDomainDTO_fleet_10) {
  ...
  async update(id: string, dto: UpdateDomainDTO_fleet_10) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_fleet_10) {
