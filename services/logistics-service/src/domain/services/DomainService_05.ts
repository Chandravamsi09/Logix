import { IDomainRepository_fleet_05 } from '../repositories/DomainRepository_05';
import { CreateDomainDTO_fleet_05, UpdateDomainDTO_fleet_05, QueryDomainFilterDTO_fleet_05 } from '../dto/DomainDTO_05';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_fleet_05 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_fleet_05) {}

  async create(dto: CreateDomainDTO_fleet_05) {
  ...
  async update(id: string, dto: UpdateDomainDTO_fleet_05) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_fleet_05) {
