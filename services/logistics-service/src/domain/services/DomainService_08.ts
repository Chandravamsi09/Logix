import { IDomainRepository_fleet_08 } from '../repositories/DomainRepository_08';
import { CreateDomainDTO_fleet_08, UpdateDomainDTO_fleet_08, QueryDomainFilterDTO_fleet_08 } from '../dto/DomainDTO_08';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_fleet_08 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_fleet_08) {}

  async create(dto: CreateDomainDTO_fleet_08) {
  ...
  async update(id: string, dto: UpdateDomainDTO_fleet_08) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_fleet_08) {
