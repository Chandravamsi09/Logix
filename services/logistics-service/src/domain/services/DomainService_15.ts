import { IDomainRepository_fleet_15 } from '../repositories/DomainRepository_15';
import { CreateDomainDTO_fleet_15, UpdateDomainDTO_fleet_15, QueryDomainFilterDTO_fleet_15 } from '../dto/DomainDTO_15';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_fleet_15 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_fleet_15) {}

  async create(dto: CreateDomainDTO_fleet_15) {
  ...
  async update(id: string, dto: UpdateDomainDTO_fleet_15) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_fleet_15) {
