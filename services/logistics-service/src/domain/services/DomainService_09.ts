import { IDomainRepository_fleet_09 } from '../repositories/DomainRepository_09';
import { CreateDomainDTO_fleet_09, UpdateDomainDTO_fleet_09, QueryDomainFilterDTO_fleet_09 } from '../dto/DomainDTO_09';
import { NotFoundException, ConflictException, Logger } from '@nexus/common';

export class DomainService_fleet_09 {
  private readonly logger = new Logger(`DomainService_${svc.domain}_${secPad}`);

  constructor(private readonly repo: IDomainRepository_fleet_09) {}

  async create(dto: CreateDomainDTO_fleet_09) {
  ...
  async update(id: string, dto: UpdateDomainDTO_fleet_09) {
    const entity = await this.getById(id);
    return this.repo.update(entity.id, dto as any);
  }

  async search(filter: QueryDomainFilterDTO_fleet_09) {
